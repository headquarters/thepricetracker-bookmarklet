...
describe("The price-node module", () => {
  ...
  it("finds a single price with simple HTML", () => {
    const dom = new JSDOM(
      `<!DOCTYPE html><p style="font-size:14px;">$9.99</p>`
    );
    global.window = dom.window;
    global.document = dom.window.document;
  ...
  it("finds multiple prices with simple HTML", () => {
    const dom = new JSDOM(
      `<!DOCTYPE html><p style="font-size:14px;">$9.99</p><p>$100.00</p>`
    );
    global.window = dom.window;
    global.document = dom.window.document;
  ...
  it("finds a price in complex HTML", () => {
    const dom = new JSDOM(
      `<!DOCTYPE html><div class="price">$<p style="font-size:14px;">9.99</p></div>`
    );
    global.window = dom.window;
    global.document = dom.window.document;
