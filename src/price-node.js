...
function getTextNodesWithPriceSymbol(node) {
  ...
  if(searchDepth > 0) {
      function _getTextNodesWithPriceSymbol(node) {
          ...
          if (node.nodeType == TEXT_NODE && node.textContent.length < TEXT_LIMIT && dollarSignRegex.test(node.textContent)) { // && node.textContent.length < TEXT_LIMIT && dollarSignRegex.test(node.textContent)) {
