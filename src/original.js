var PriceTracker = (function() {
  ...
  panelHTML +=
    '<span id="price-tracker-current-image">1</span> of <span id="price-tracker-image-total">1</span>';
  ...
  panelHTML +=
    '<div id="price-tracker-currently-selected-price" data-priceindex="0"></div>';
  ...
  self.setupPanel = function() {
    ...
    if (priceNodeCollection.length > 0) {
      ...
      var $price = $panel
        .find("#price-tracker-currently-selected-price")
        .data("priceindex", 0)
        .text(priceNodeCollection[0].price);

      var priceFloat = parseFloat(
        priceNodeCollection[0].price.replace("$", "")
      );

      var discountedPrice = Math.floor(
        priceFloat - Math.ceil(priceFloat * 0.1)
      );
      ...
      $button.text("Close");
      $button.before(
        '<div class="error">No prices were found on this page.</div>'
      );
    }
  };
    ...
    if (document.images.length > 0) {
      ...
      images.sort(function(image1, image2) {
        ...
        return (
          ...
          image2OffsetLeft -
          image2OffsetTop -
          (image1.width + image1.height - image1OffsetLeft - image1OffsetTop)
        );
      });
      ...
      $previousImage.add($nextImage).on("click.PriceTracker", function(e) {
        ...
        var currentImageIndex =
          parseInt($("#price-tracker-current-image").text()) - 1;
  ...
  self.bindEvents = function() {
    ...
    //Save
    $panel
      .find("#price-tracker-actions button")
      .on("click.PriceTracker", function(e) {
        ...
        var currentPrice = $("#price-tracker-currently-selected-price")
          .text()
          .replace("$", "");
        var targetPrice = $("#price-tracker-notify-me input").val();
        var index = $("#price-tracker-currently-selected-price").data(
          "priceindex"
        );
        ...
        $button
          ...
          .before(
            '<img src="https://thepricetracker.com/images/loading.gif" alt="" />'
          );
        ...
        $.ajax({
          ...
          success: function(data, txt, xhr) {
            ...
            if (data.status === "success") {
              ...
              if ($button.next("p").length) {
                $button
                  .next("p")
                  .replaceWith('<p class="error">' + resultMessage + "</p>");
              } else {
          ...
          error: function(xhr, txt, err) {
            resultMessage =
              "Sorry, an error has occurred while saving. Please try again later.";
            $button.removeAttr("disabled");
            $button
              .text("Start Tracking")
              .prev("img")
  ...
  self.finish = function() {
    ...
    currentPriceIndex = $("#price-tracker-currently-selected-price").data(
      "priceindex"
    );
